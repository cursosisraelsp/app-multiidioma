...
class CimusRepository {
    fun getMiniScreens(
        detailScreenContent:  List<MiniScreenData>,
        languageViewModel: String
    ): List<@Composable (Modifier) -> Unit> {
        return listOf(
            { modifier -> MiniScreen0(data = detailScreenContent[0], languageViewModel, modifier) },
            { modifier -> MiniScreen1(data = detailScreenContent[1], languageViewModel, modifier) },
            { modifier -> MiniScreen2(data = detailScreenContent[2], languageViewModel, modifier) }
        )
    }
}
